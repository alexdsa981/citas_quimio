----------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------ DATOS PACIENTE DESDE PRESUPUESTO ------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------------------------
...
DATEDIFF(YEAR, Paciente.FechaNacimiento, GETDATE()) - CASE 
	WHEN MONTH(Paciente.FechaNacimiento) > MONTH(GETDATE()) OR (MONTH(Paciente.FechaNacimiento) = MONTH(GETDATE()) AND DAY(Paciente.FechaNacimiento) > DAY(GETDATE())) 
	THEN 1 
    ELSE 0 
    END AS Edad,
...
FROM SS_AD_Garantia
LEFT JOIN PersonaMast AS Paciente ON Paciente.Persona = SS_AD_Garantia.IdPaciente
LEFT JOIN CM_CO_TablaMaestroDetalle ON CM_CO_TablaMaestroDetalle.IdCodigo = SS_AD_Garantia.TipoPaciente
	AND CM_CO_TablaMaestroDetalle.IdTablaMaestro = 106
LEFT JOIN PersonaMast AS Aseguradora ON Aseguradora.Persona = SS_AD_Garantia.IdEmpresaAseguradora
LEFT JOIN PersonaMast AS Empleadora ON Empleadora.Persona = SS_AD_Garantia.IdEmpresaEmpleadora
--WHERE SS_AD_Garantia.IdGarantia = 1
